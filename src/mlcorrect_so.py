...
# =============================================================================
#
# This scripts trains a Delta_SO_ML model and uses it for predictions
# https://doi.org/10.1039/D3CP05556F
#
# Run:
# mlcorrect_so.py [input files/directories] [options]
#
...
########## RE-DEFINED FUNCTIONS THAT ALSO EXIST IN mlcorrect_corr.py ##########

def build_model(train_dataset, n_nodes_1: int, n_nodes_2: int, drop, opt: str, activ: str) -> keras.Sequential:
    ...
    elif opt == 'adadelta': optimizer = keras.optimizers.Adadelta()
    elif opt == 'adafactor': optimizer = keras.optimizers.Adafactor()
    elif opt == 'adagrad': optimizer = keras.optimizers.Adagrad()
    elif opt == 'adamw': optimizer = keras.optimizers.AdamW()
    elif opt == 'adamax': optimizer = keras.optimizers.Adamax()
    elif opt == 'ftrl': optimizer = keras.optimizers.Ftrl()
    elif opt == 'lion': optimizer = keras.optimizers.Lion()
    elif opt == 'nadam': optimizer = keras.optimizers.Nadam()
    else:
        print("ERROR: Unknown optimizer selected in build_model()!")
        exit()

    # the loss function is defined with command-line keyword --loss (default is mse)
    if loss_is_custom:
        model.compile(loss=custom_loss, optimizer=optimizer, metrics=[max_error_neg, max_error_pos, mean_error, 'mae', 'mse', rmse, standard_deviation])
    elif loss_function.lower() in ['rmsd', 'rmse']:
        model.compile(loss=rmse, optimizer=optimizer, metrics=[max_error_neg, max_error_pos, mean_error, 'mae', 'mse', rmse, standard_deviation])
    else:
        model.compile(loss=loss_function, optimizer=optimizer, metrics=[max_error_neg, max_error_pos, mean_error, 'mae', 'mse', rmse, standard_deviation])
...
def evaluate_labels(labels: pd.Series) -> dict:
    ...
    # labels contain the target of the data set, which is reference - trial (here SO - nonrel)
    ...
    rmse = np.sqrt(mse)
    sd = np.std(errors)
    sd_si = np.sqrt(sum([(e-me)**2 for e in errors]) / (len(errors)-1))

    return {'MAX_neg': max_neg, 'MAX_pos': max_pos, 'ME': me, 'MAE': mae, 'MSE': mse, 'RMSE': rmse, 'SD': sd, 'SD_SI': sd_si}
...
def print_details(seg_dataset: pd.DataFrame, normed_data: pd.DataFrame , labels: pd.Series, nuc: str, name='?'):
    """Print a list of the true and predicted SO contributions and some other quantities of a training run.
    ...
    Args:
        ...
        nuc: Nucleus of interest (must be 'h' or 'c').
    ...
    print("Number & Category & SO_true & SO_predicted & shift_non-rel & shift_rel_true & shift_rel_predicted & prediction_error")

    for i in range(labels.size):
        if nuc == 'h':
            if seg_dataset['no_HA_1'].iat[i] + seg_dataset['no_HA_2'].iat[i] > 0: category = 1
            elif seg_dataset['no_HA_3'].iat[i] > 0: category = 2
            else: category = 3
        elif nuc == 'c':
            if seg_dataset['no_HA_1'].iat[i] > 0: category = 1
            elif seg_dataset['no_HA_2'].iat[i] > 0: category = 2
            else: category = 3
        print("{:6d} & {:1d} & {:13.8f} & {:13.8f} & {:13.8f} & {:13.8f} & {:13.8f} & {:13.8f}".format(
            labels.index[i], category, labels.iat[i], predictions[i], seg_dataset['shift_low'].iat[i],
            seg_dataset['shift_low'].iat[i] + labels.iat[i], seg_dataset['shift_low'].iat[i] + predictions[i],
            predictions[i] - labels.iat[i]
        ))
...
########## NEW FUNCTIONS THAT DO NOT EXIST IN mlcorrect_corr.py ##########

def custom_loss(y_true, y_pred) -> float:
    """Defines a somewhat custom loss function with three adjustable parameters.

    Functional form: # x^f(x) with f(x) = alpha * exp(-beta * x^2) + gamma
    Parameters alpha, beta, and gamma are defined with the comman-line keyword --loss

    Args:
        y_true: The true reference values.
        y_pred: The actual predicted values.
    
    Returns:
        The loss function value (float).
    """
    ...
    proc_errors = tf.math.abs(errors) * (alpha * tf.math.exp(-1 * beta * (errors ** 2)) + gamma)
    quantity = tf.reduce_mean(proc_errors)
...
if __name__ == '__main__':
    ...
    workdir = os.getcwd()
    print("MLCORRECT was called via:\n{}\nin:\n{}\n".format(' '.join(sys.argv), workdir))
    ...
    parser.add_argument('--nneurons1', type=int, help='Number of neurons in 1st layer, default: 384 (1H), 300 (13C).', metavar='Nneu')
    parser.add_argument('--nneurons2', type=int, help='Number of neurons in 2nd layer, default: 80 (1H), 12 (13C).', metavar='Nneu')
    parser.add_argument('--dropout', type=float, nargs=2, help='Dropout rate of 1st and 2nd layer, default: 0.15/0.10 (1H), 0.10/0.00 (13C).', metavar='D')
    ...
    parser.add_argument('--activation', type=str, help='Activation function used in the ML training process, default: sigmoid (1H), gelu (13C).', metavar='func')
    ...
    parser.add_argument('--loss', type=str, default='rmse', help='Choose a loss function (from selection or "custom_alpha_beta_gamma") (ONLY USED FOR TESTING!).', metavar='loss')
    ...
    if args.nneurons1 is None:
        if nuc == 'h': n_neurons_1 = 384
        if nuc == 'c': n_neurons_1 = 300
    else: n_neurons_1 = args.nneurons1
    
    if args.nneurons2 is None:
        if nuc == 'h': n_neurons_2 = 80
        if nuc == 'c': n_neurons_2 = 12
    else: n_neurons_2 = args.nneurons2

    if args.dropout is None:
        if nuc == 'h': dropout_rate = [0.15, 0.1]
        if nuc == 'c': dropout_rate = [0.1, 0.0]
    else: dropout_rate = args.dropout

    if args.activation is None:
        if nuc == 'h': activation = 'sigmoid'
        if nuc == 'c': activation = 'gelu'
    else: activation = args.activation
    ...
    # get the loss function input (this is prone to bugs, so just use it for testing!)
    loss_function = args.loss
    loss_is_custom = False
    loss_params = (None, None, None)
    tmp = loss_function.split('_')
    if tmp[0] == 'custom':
        loss_params = (float(tmp[1]), float(tmp[2]), float(tmp[3]))
        loss_is_custom = True
    ...
    keras.utils.set_random_seed(random_seed)   # this works with tf version 2.12
    ...
    if do_train:
        ...
        descriptors = {
            'geometric': {
                'h': ['CN(X)', 'dist_HX', 'no_HYH', 'no_HYC', 'no_HYN', 'no_HYO', 'no_HYS', 'no_HYCl', 'no_HYSe', 'no_HYBr', 'no_HYTe', 'no_HYI'],
                'c': ['CN(X)', 'no_CH', 'no_CC', 'no_CN', 'no_CO', 'no_CS', 'no_CCl', 'no_CSe', 'no_CBr', 'no_CTe', 'no_CI', 'no_CYH', 'no_CYC', 'no_CYN', 'no_CYO', 'no_CYS', 'no_CYCl', 'no_CYSe', 'no_CYBr', 'no_CYTe', 'no_CYI']
            },
            'electronic': {
            ...
            'magnetic': {
                'h': ['shift_low', 'shielding_dia', 'shielding_para', 'span', 'skew', 'asymmetry', 'anisotropy'],
                'c': ['shift_low', 'shielding_dia', 'shielding_para', 'span', 'skew', 'asymmetry', 'anisotropy']
            },
            'symmetric': {
                'h': list(addso.acsf_labels) + list(addso.soap_labels_reduced),
                'c': list(addso.acsf_labels) + list(addso.soap_labels_reduced)
            },
            'acsf': {
                'h': list(addso.acsf_labels),
                'c': list(addso.acsf_labels)
            }
        }
        ...
        if not exclude_descriptors:
            ...
            slope, intercept, r_squared = linear_fit(train_dataset['shift_low'], train_dataset['so_contribution'])
            print("Training data fitted to linear function with:\nSlope:     {:10.6f}\nIntercept: {:10.6f}\nR^2:       {:10.6f}\n".format(slope, intercept, r_squared))

            lr_train_target = train_dataset['so_contribution'] + train_dataset['shift_low']
            lr_train_unscaled = train_dataset['shift_low']
            lr_train_scaled = scale_linear(train_dataset['shift_low'], slope, intercept)

            lr_test_target = test_dataset['so_contribution'] + test_dataset['shift_low']
        ...
        # Get the overall statistics and exclude column 'so_contribution' (columns are transposed to rows)
        train_stats = train_dataset.describe()
        train_stats.pop('so_contribution')
        ...
        train_labels = train_dataset.pop('so_contribution')
        test_labels = test_dataset.pop('so_contribution')
        ...
        print("Mean Squared Error / ppm^2    (MSE)    : {:9.4f}      {:9.4f} ({:.1f}% reduction, sqrt = {:.1f}%)".format(stats_train_old['MSE'], stats_train_new['MSE'], (1 - stats_train_new['MSE']/stats_train_old['MSE'])*100, (1 - np.sqrt(stats_train_new['MSE']/stats_train_old['MSE']))*100))
        ...
        print("Standard Deviation / ppm      (SD)     : {:9.4f}      {:9.4f}\n".format(stats_train_old['SD_SI'], stats_train_new['SD_SI']))
        ...
        print("Mean Squared Error / ppm^2    (MSE)    : {:9.4f}      {:9.4f} ({:.1f}% reduction, sqrt = {:.1f}%)".format(stats_test_old['MSE'], stats_test_new['MSE'], (1 - stats_test_new['MSE']/stats_test_old['MSE'])*100, (1 - np.sqrt(stats_test_new['MSE']/stats_test_old['MSE']))*100))
        ...
        print("Standard Deviation / ppm      (SD)     : {:9.4f}      {:9.4f}\n\n".format(stats_test_old['SD_SI'], stats_test_new['SD_SI']))

        # optionally print all train/test set data points (e.g. for use in plots)
        # metrics calculated from this list should be identical to the ones above under *** TRAINIG/TEST SET ***
        if 'train' in print_testdetails: print_details(train_dataset, normed_train_data, train_labels, nuc, name='training')
        if 'test' in print_testdetails: print_details(test_dataset, normed_test_data, test_labels, nuc, name='test')
    ...
    if do_predict:
        ...
        print("Started mlcorrect in prediction mode for NMR nucleus: {}\n".format(nuc.upper()))
        ...
        sample_labels = sample_data.pop('so_contribution')
        ...
        for i in sample_labels:
            # Check if the first element is not a float or is NaN (which is a float, but does not equal itself)
            if not isinstance(i, float) or i != i:
                has_highlevel = False
                break
        ...
        for i, at in enumerate(atnums):
            ...
            datapoint = {
                ...
                'so_contribution_predicted': sample_predictions[i],
                'shift_so_predicted': sample_data['shift_low'].iat[i] + sample_predictions[i]
            }
            if has_highlevel:
                datapoint['so_contribution_true'] = sample_labels.iat[i]
                datapoint['shift_so_true'] = datapoint['shift_low'] + sample_labels.iat[i]
            final_data.append(datapoint)
        
        # Print the final prediction results
        print("\n*** FINAL SAMPLE PREDICTION RESULTS ***\n")
        if has_highlevel:
            print("Number   Atom   Calculated shift   Predicted SO-contribution   Predicted shift (SO)   True SO-contribution   True shift (SO)   Prediction error")
            for at in final_data:
                print("{:4d}     {:3d}         {:7.4f}                {:7.4f}                  {:7.4f}                {:7.4f}              {:7.4f}           {:7.4f}".format(
                    at['index'], at['atom'], at['shift_low'], at['so_contribution_predicted'], at['shift_so_predicted'], at['so_contribution_true'], at['shift_so_true'], at['so_contribution_predicted'] - at['so_contribution_true']
                ))
        else:
            print("Number   Atom   Calculated shift   Predicted SO-contribution   Predicted shift (SO)")
            for at in final_data:
                print("{:4d}     {:3d}         {:7.4f}                {:7.4f}                  {:7.4f}".format(
                    at['index'], at['atom'], at['shift_low'], at['so_contribution_predicted'], at['shift_so_predicted']
                ))

        # Write an output file containing the ML-predicted SO contributions and shifts
        with open(os.path.join(workdir, "corrected_" + os.path.basename(sample_path)), 'w') as out:
            if nuc == 'h': out.write("# ML-predicted SO contributions to 1H NMR chemical shifts for data in {}\n".format(sample_path))
            if nuc == 'c': out.write("# ML-corrected SO contributions to 13C NMR chemical shifts for data in {}\n".format(sample_path))
            if has_highlevel:
                out.write("# atom shift_calculated so_contribution_predicted so_contribution_true\n")
                for at in final_data:
                    out.write("{} {} {} {}\n".format(at['atom'], at['shift_low'], at['so_contribution_predicted'], at['so_contribution_true']))
            else:
                out.write("# atom shift_calculated so_contribution_predicted\n")
                for at in final_data:
                    out.write("{} {} {}\n".format(at['atom'], at['shift_low'], at['so_contribution_predicted']))
